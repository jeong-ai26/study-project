...
### 오늘의 코드/실험 스니펫 (선택)
- 원하는 조건의 입력값을 받고 싶을 때
  ```python
  x = input('조건에 맞는 값을 입력해주세요.: ')
  ...
  while True:
    if [원하는 조건]:
      ...
      x = input('상황1로 입력하면 안됩니다.: ')
    elif [상황2]:
      x = input('상황2로 입력하면 안됩니다.: ')
    elif [상황3]:
      x = input('상황3로 입력하면 안됩니다.: ')
    else:
      x = input('잘못된 값을 입력하였습니다.: ')
  ```
...
- 정수만을 받고 싶은 코드를 짤 때 lstrip() 함수를 쓰면 '--10000'같은 잘못된 입력도 숫자로 인식하는 문제가 있다. -> replace()로 해결
  잘못된 예:
  ```python
...
- 파일이 같은 디렉터리에 있는지 확인하고 싶을 때 pathlib 내장 모듈을 사용하면 된다.
  ```python
